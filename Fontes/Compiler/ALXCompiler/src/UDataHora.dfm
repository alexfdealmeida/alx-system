inherited frmDataHora: TfrmDataHora
  ActiveControl = edtData
  Caption = 'Informe a data e a hora de compila'#231#227'o dos m'#243'dulos'
  ClientHeight = 133
  ClientWidth = 387
  ExplicitWidth = 393
  ExplicitHeight = 161
  PixelsPerInch = 96
  TextHeight = 13
  inherited stbMaster: TAdvOfficeStatusBar
    Top = 114
    Width = 387
    ExplicitTop = 114
    ExplicitWidth = 387
  end
  object pnlBotoes: TAdvSmoothPanel [1]
    Left = 0
    Top = 73
    Width = 387
    Height = 41
    Cursor = crDefault
    Caption.HTMLFont.Charset = DEFAULT_CHARSET
    Caption.HTMLFont.Color = clWindowText
    Caption.HTMLFont.Height = -11
    Caption.HTMLFont.Name = 'Tahoma'
    Caption.HTMLFont.Style = []
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWindowText
    Caption.Font.Height = -16
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.ColorStart = 5978398
    Caption.ColorEnd = 5978398
    Caption.LineColor = 5978398
    Fill.Color = 16643823
    Fill.ColorTo = 15784647
    Fill.ColorMirror = clNone
    Fill.ColorMirrorTo = clNone
    Fill.GradientMirrorType = gtVertical
    Fill.BorderColor = 13087391
    Fill.Rounding = 10
    Fill.RoundingType = rtNone
    Fill.ShadowOffset = 0
    Fill.Glow = gmRadial
    Version = '1.0.9.2'
    Align = alBottom
    TabOrder = 1
    DesignSize = (
      387
      41)
    object btnGravar: TBitBtn
      Left = 233
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Confirma dados'
      Anchors = [akRight, akBottom]
      Caption = '&Confirmar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = btnGravarClick
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      NumGlyphs = 2
    end
    object btnCancelar: TBitBtn
      Left = 309
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Cancela'
      Anchors = [akRight, akBottom]
      Caption = 'Ca&ncelar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = btnCancelarClick
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
  end
  object AdvSmoothPanel1: TAdvSmoothPanel [2]
    Left = 0
    Top = 0
    Width = 387
    Height = 73
    Cursor = crDefault
    Caption.HTMLFont.Charset = DEFAULT_CHARSET
    Caption.HTMLFont.Color = clWindowText
    Caption.HTMLFont.Height = -11
    Caption.HTMLFont.Name = 'Tahoma'
    Caption.HTMLFont.Style = []
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWindowText
    Caption.Font.Height = -16
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.ColorStart = 5978398
    Caption.ColorEnd = 5978398
    Caption.LineColor = 5978398
    Fill.Color = 16643823
    Fill.ColorTo = 15784647
    Fill.ColorMirror = clNone
    Fill.ColorMirrorTo = clNone
    Fill.GradientMirrorType = gtVertical
    Fill.BorderColor = 13087391
    Fill.Rounding = 10
    Fill.RoundingType = rtNone
    Fill.ShadowOffset = 0
    Fill.Glow = gmRadial
    Version = '1.0.9.2'
    Align = alClient
    TabOrder = 3
    object grpAdaptador: TAdvGroupBox
      Left = 10
      Top = 10
      Width = 175
      Height = 49
      Caption = 'Adaptador'
      TabOrder = 0
      object edtData: TJvDateEdit
        Left = 5
        Top = 20
        Width = 100
        Height = 19
        Hint = 'Informe o usu'#225'rio'
        ButtonWidth = 20
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnExit = edtDataExit
      end
      object edtHora: TMaskEdit
        Left = 110
        Top = 20
        Width = 60
        Height = 19
        Hint = 'Informe a senha'
        EditMask = '!99:99:99;1;_'
        MaxLength = 8
        ParentShowHint = False
        ShowHint = True
        TabOrder = 1
        Text = '  :  :  '
      end
    end
    object grpModulos: TAdvGroupBox
      Left = 195
      Top = 10
      Width = 175
      Height = 49
      Caption = 'Demais m'#243'dulos'
      TabOrder = 1
      object edtDataAux: TJvDateEdit
        Left = 5
        Top = 20
        Width = 100
        Height = 19
        Hint = 'Informe o usu'#225'rio'
        ButtonWidth = 20
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
      end
      object edtHoraAux: TMaskEdit
        Left = 110
        Top = 20
        Width = 60
        Height = 19
        Hint = 'Informe a senha'
        EditMask = '!99:99:99;1;_'
        MaxLength = 8
        ParentShowHint = False
        ShowHint = True
        TabOrder = 1
        Text = '  :  :  '
      end
    end
  end
  inherited stbStylerMaster: TAdvOfficeStatusBarOfficeStyler
    Left = 168
    Top = 79
  end
  inherited ImageListMaster: TImageList
    Left = 128
    Top = 79
  end
  object JvEnterAsTab1: TJvEnterAsTab
    Left = 78
    Top = 79
  end
end
