{ $Id: GUITestRunner.pas,v 1.121 2004/04/29 18:52:45 juanco Exp $ }
{: DUnit: An XTreme testing framework for Delphi programs.
   @author  The DUnit Group.
   @version $Revision: 1.121 $ 2001/03/08 uberto
}
(*
...
begin
  ...
  begin
    if NodeToTest(node) as IUnknown = (test as IUnknown) then // cast to IUnknown required because of the COM nature
    begin
      FoundNode := node;
      break;
    end;
...
begin
  if Suite <> nil then
    Suite.SaveConfiguration(IniFileName, UseRegistryAction.Checked);
...
begin
  ...
  begin
    ...
    begin
      ...
      if Suite.CountEnabledTestCases = 0 then
...
begin
  if Suite <> nil then
    Suite.LoadConfiguration(IniFileName, UseRegistryAction.Checked);
end;
