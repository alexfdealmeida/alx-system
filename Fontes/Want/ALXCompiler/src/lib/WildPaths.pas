...
begin
  ...
  //Alex
  //Assert(Pos('//', Result) <= 1);
  Assert(Pos('//', Result) <= 1, Path1 + Path2 + Result);
end;
...
begin
  ...
  begin
    //Alex -  Tratamento para funcionar no Win7, devido o arquivos de programa conter o texto (x86)
    //if Pos(InvalidPathChars[i], Path) <> 0 then
    //  raise EPathException.Create('invalid path chars in ' + Path);
    if (Pos(InvalidPathChars[i], Path) <> 0) and (Pos('(x86)', Path) < 0) then
      raise EPathException.Create('invalid path chars in ' + Path + ' Posição inválida: ' + IntToStr(Pos('(x86)', Path)));
  end;
end;
...
begin
  ...
  if (Path <> '') and (Path <> CurrentDir) then
    ...
    if not Result and Verify then
      raise EFileOpException.CreateFmt('Could not change to directory "%s"',[Path]); //Alex, verificar esta mensagem, pois a mesma não permite informar espaço nos diretórios
end;
