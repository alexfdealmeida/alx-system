...
type
  ...
  protected
    ...
    procedure Run(CmdLine, WorkDir: string);
...
begin
  //Result := '"' + WildPaths.ToSystemPath(Executable) + '"'; Alex
...
begin
  ...
  //Alex - Tratamento para executar o fde.exe apenas se o mesmo existir na máquina
  if Pos('fde', CmdLine) > 0 then
  begin
    lDirSystem := SysSystemDir;

    lDirLocal  := ExtractFileDir(ParamStr(0));

    if not (FileExists(lDirSystem + '\fde.exe')) and not (FileExists(lDirLocal + '\fde.exe')) then
    begin
      Log(vlWarnings, 'Arquivo fde.exe nao localizado em nenhum dos seguintes diretorios: ');
      Log(vlWarnings, ' - ' + lDirSystem);
      Log(vlWarnings, ' - ' + lDirLocal);
      Exit;
    end;
  end;

  Run(CmdLine, ToSystemPath(ToPath(GetBasePath)));
end;
...
procedure TCustomExecTask.Run(CmdLine, WorkDir: string);
var
  Child    :TChildProcess;
  ExitCode :Integer;
begin
