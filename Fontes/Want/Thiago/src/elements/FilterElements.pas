...
begin
  ...
  if FSL.Count >= FLines then
     for i := FSL.Count - FLines to FSL.Count - 1
        do Result := Result + FSL[i] + #13#10;
end;
...
begin
  if not Assigned(FContainsList) then FContainsList := TList.Create;
...
begin
  ...
  begin
    ...
    begin
      if regex.Match(FRegExp, FSL[i]) then 
      begin
        Result := Result + FSL[i] + #13#10;
        continue;
      end;
    ...
    begin
      ...
      begin
        if regex.Match((Children[j] as TLineContainsRegexpRegexpElement).pattern, fSL[i]) then 
        begin
          Result := Result + FSL[i] + #13#10;
          break;  // break from inside for loop to next line in SL
        end;
...
begin
  inherited;
  if ChildCount = 0 then RequireAttribute('regexp');
end;
...
begin
  ...
  for i := 0 to FSL.Count - 1 do FSL[i] := FPrefix + FSL[i];
  Result := FSL.Text; 
end;
...
begin
  inherited;
  if ChildCount = 0 then RequireAttribute('comment');
end;
...
begin
  bSpaces := '';
  for i := 1 to FTabLength do bSpaces := bSpaces + ' ';
...
begin
  if not Assigned(FTokenList) then FTokenList := TList.Create;
