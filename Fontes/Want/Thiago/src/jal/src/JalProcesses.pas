...
const
  rcs_id :string = '#(@)$Id: JalProcesses.pas,v 1.3 2003/06/05 03:39:01 barnson Exp $';
...
{$ENDIF VER130}
  ...
  public
    ...
    procedure Run(CmdLine, WorkDir: string);  virtual; abstract;
  ...
  protected
    ...
    function  Launch(const CmdLine, WorkDir: string; hInput, hOutput, hError: THandle): THandle;
  ...
  public
    ...
    procedure Run(CmdLine, WorkDir: string);  override;
...
procedure TWin32ChildProcess.Run(CmdLine, WorkDir: string);
begin
  try
    hChild := Launch(CmdLine, WorkDir, hInputRead, hOutputWrite, hOutputWrite);
  finally
...
function TWin32ChildProcess.Launch(const CmdLine, WorkDir: string; hInput, hOutput, hError: THandle): THandle;
var
...
begin
  ...
  Success := CreateProcess(nil, PChar(CmdLine), nil, nil, True,
    NORMAL_PRIORITY_CLASS, nil, PChar(WorkDir), StartupInfo,
    ProcessInfo);
  if not Success then
