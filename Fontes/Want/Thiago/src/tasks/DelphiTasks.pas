...
const
  DelphiRegRoot  = 'SOFTWARE\Borland';
...
type
  ...
  protected
    ...
    function BuildArguments(Parallel: Boolean): string; override;
  ...
  published
    ...
    property td32           :boolean read FTD32           write FTD32           default false;
    ...
    property dversion       :string read FDVersion write FDVersion;
  end;
  ...
  TTD32Element = class(TOptionElement)
  private
    FValue: boolean;
  published
  public
...
begin
  ...
  if V = '' then
  begin
    WantUtils.GetEnvironmentVar('delphi_version', V, false);
  end;
  if V = '' then begin
     V := 'BDS\5,BDS\4,Delphi\9,Delphi\8,Delphi\7,Delphi\6,Delphi\5,Delphi\4';
  end;
...
begin
  with FindDelphi(toolversion) do
  begin
    FVersionFound := Version;
    FDelphiDir    := Directory;
    FToolPath     := ToolPath;
    DecimalSeparator := '.';
    FVersionNumber := StrToFloat(Copy(Version, Pos('\', Version)+1, Length(Version)));
    GetFormatSettings();
  end;
  if FToolPath = '' then
...
begin
  ...
  try
    toolversion := dversion;
    inherited Execute(Parallel);

    if FTempDcu then
    begin
//      DeleteFiles(FindFiles('*', dcuoutput), True);
//      RemoveDir(dcuoutput);
    end;
...
function TDelphiCompileTask.BuildArguments(Parallel: Boolean): string;
var
  Sources: TPaths;
  d      : Integer;
  a      : Integer;
  ...
  libPath :string;
  path    :string;
  pardcu  : array[0..MAX_PATH] of char;
begin
  Result := '';
  ...
  if (dcuoutput = '') and Parallel then
  begin
    Log(vlVerbose, 'Putting dcuoutput because of Parallel build.');
    dcuoutput := exeoutput + '\_dcus\' + Copy(source, 1, Pos('.', source)-1);
    MakeDir(dcuoutput);
    FTempDcu := True;
  end;
  ...
  begin
    ...
    Result := Result + PathOpt('N0', dcuoutput);
    FIncludePaths.Includes.Add(dcuoutput);
  end;
  ...
  Result := Result + ' ' + inherited BuildArguments(Parallel);
      ...
      try
        ...
        begin
          ...
          begin
            ...
            PS[p] := StringReplace(PS[p], '$(BDS)', DelphiDir, [rfReplaceAll, rfIgnoreCase]);

            if (Pos('$(BDS', PS[p]) > 0) then // Ignore others BDS paths
              Continue;
...
function TResourceCompileTask.BuildArguments(Parallel: Boolean): string;
begin
  Result := inherited BuildArguments(Parallel);
