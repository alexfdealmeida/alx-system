...
type
  ...
  protected
    ...
    function BuildArguments(Parallel: Boolean)  :string; virtual;
    function BuildCmdLine(Parallel: Boolean)    :string; virtual;
...
begin
  ...
  Log(vlDebug, 'arguments='  + BuildArguments(Parallel));
...
function TCustomExecTask.BuildCmdLine(Parallel: Boolean): string;
begin
  Result := Trim(Trim(BuildExecutable) + ' ' + BuildArguments(Parallel));
end;

function TCustomExecTask.BuildArguments(Parallel: Boolean): string;
var
  i: Integer;
begin
  Result := '';
